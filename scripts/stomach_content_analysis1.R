...
count.dat <- filter(long.prey.df, Measurement == "Count" & Prey_group == "Chironomid")
adonis2(Value ~ Species, data = count.dat)

count.dat <- filter(long.prey.df, Measurement == "Count" & spp_ID != "ARCS_106")
levins.df <- as.data.frame(count.dat %>% 
                             group_by(Species, Prey_group) %>% 
                             summarise(Total = sum(Value)) %>% 
                         pivot_wider(id_cols = Prey_group, names_from = Species, values_from = Total, values_fill = 0))
rownames(levins.df) <- levins.df$Prey_group
levins.df <- levins.df[,-1]

niche.width(levins.df, method = "levins")
niche.overlap(levins.df, method = "levins")
niche.overlap(levins.df, method = "schoener")
str(levins.df)
levins.sampleInfor <- count.dat$Prey_group

levins.Bn(df = levins.df, R = 4, sampleInfo = levins.sampleInfor)
