...
samples2 <- lw22 %>% 
  ...
  select(c(species, lg_group, ID))
samples <- rbind(samples1, samples2)
samples <- samples %>% unite(spp_ID, c(species,ID), sep = "_", remove = FALSE)
lens.samples <- inner_join(select(lw22, c(spp_ID, species, length_mm, lg_group)), select(samples, c(species, spp_ID, lg_group)), by = c("spp_ID", "species", "lg_group"))
